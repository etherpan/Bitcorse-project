...
import {BNB_TICKER, BUL_TICKER, DSHARE_TICKER, BTC_TICKER, ZAPPER_ROUTER_ADDR} from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  else if (zappingToken === BUL_TICKER) token = bombFinance.BCE;
  else if (zappingToken === DSHARE_TICKER) token = bombFinance.MCORSE;
