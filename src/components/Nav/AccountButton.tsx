...
const AccountButton: React.FC<AccountButtonProps> = ({text}) => {
  ...
  return (
    <div>
      {!account ? (
        <Button onClick={handleWalletProviderOpen} className="shinyButtonSecondary accountButton-address">
          {buttonText}
        </Button>
      ) : (
        <Button onClick={onPresentAccountModal} className="shinyButtonSecondary accountButton-address">
