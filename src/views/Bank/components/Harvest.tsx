...
const Harvest: React.FC<HarvestProps> = ({bank}) => {
  ...
  console.log('tokenPriceInDollars', earnings.toString());
  const {onReward} = useHarvest(bank);
  const bombStats = useBulStats();
  const tShareStats = useShareStats();

  const tokenName = bank.earnTokenName === 'MCORSE' ? 'MCORSE' : 'BCE';
  const tokenStats = bank.earnTokenName === 'MCORSE' ? tShareStats : bombStats;
