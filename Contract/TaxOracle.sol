...
contract TaxOracle is Ownable {
    ...
    constructor(
        ...
        require(_bce != address(0), "bce address cannot be 0");
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(bce), "token needs to be bce");
        uint256 bceBalance = bce.balanceOf(pair);
        uint256 wbnbBalance = wbnb.balanceOf(pair);
        return uint144(bceBalance.mul(_amountIn).div(wbnbBalance));
    }

    function getBceBalance() external view returns (uint256) {
	return bce.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 bceBalance = bce.balanceOf(pair);
        uint256 wbnbBalance = wbnb.balanceOf(pair);
        return bceBalance.mul(1e18).div(wbnbBalance);
    }


    function setBce(address _bce) external onlyOwner {
        require(_bce != address(0), "bce address cannot be 0");
        bce = IERC20(_bce);
    }
