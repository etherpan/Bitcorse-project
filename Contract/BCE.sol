 ...
 *
    __________ __  __                                      
    \______   \__|/  |_  ____   ____ _______  ______ ____  
    |   |  _/  |   __\/ ___\ / __ \\_  __ \/  ___// __ \ 
    |   |   \  ||  | \  \___(  \_\ )|  | \/\___ \\  ___/_
    /______  /__||__|  \___  /\____/ |__|  /____  \\___  /
            \/              \/                   \/     \/ 
...
contract Bce is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_BCE_REWARD_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BCE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Bitcorse", "BCE") {
        // Mints 1 BCE to contract creator for initial pool setup
    ...
    function _getBcePrice() internal view returns (uint256 _bcePrice) {
        try IOracle(bceOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bce: failed to fetch BCE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _bcePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_bcePrice >= taxTiersTwaps[tierId]) {
    ...
    function setBceOracle(address _bceOracle) public onlyOperatorOrTaxOffice {
        require(_bceOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BCE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BCE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBcePrice = _getBcePrice();
            currentTaxRate = _updateTaxRate(currentBcePrice);
            if (currentBcePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _bceRewardPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_bceRewardPool != address(0), "!_bceRewardPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_bceRewardPool, INITIAL_BCE_REWARD_POOL_DISTRIBUTION);
