...
contract McorseWrapper {
    ...
    function stake(uint256 amount) public virtual {
        ...
        mcorse.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public virtual {
        uint256 memberMcorse = _balances[msg.sender];
        require(memberMcorse >= amount, "Boardroom: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = memberMcorse.sub(amount);
        mcorse.safeTransfer(msg.sender, amount);
    }
}


contract Boardroom is McorseWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        BoardroomSnapshot memory genesisSnapshot = BoardroomSnapshot({time: block.number, rewardReceived: 0, rewardPerMcorse: 0});
    ...
    function getBcePrice() external view returns (uint256) {
        return treasury.getBcePrice();
    }

    // =========== Member getters

    function rewardPerMcorse() public view returns (uint256) {
        return getLatestSnapshot().rewardPerMcorse;
    }

    function earned(address member) public view returns (uint256) {
        uint256 latestRPS = getLatestSnapshot().rewardPerMcorse;
        uint256 storedRPS = getLastSnapshotOf(member).rewardPerMcorse;
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            bce.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 prevRPS = getLatestSnapshot().rewardPerMcorse;
        uint256 nextRPS = prevRPS.add(amount.mul(1e18).div(totalSupply()));

        BoardroomSnapshot memory newSnapshot = BoardroomSnapshot({time: block.number, rewardReceived: amount, rewardPerMcorse: nextRPS});
        boardroomHistory.push(newSnapshot);

        bce.safeTransferFrom(msg.sender, address(this), amount);
